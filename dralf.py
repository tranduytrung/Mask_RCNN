...
class AutoGeneratedDataset:
    ...
    def __getitem__(self, index):
        ...
        if self.depth:
            ...
            # drop channel
            drop_mode = np.random.randint(3)
            # mode = 0 -> no drop
            if drop_mode == 0:
                np_image = np.concatenate((np_rgb, np_d), axis=2)
            # mode = 1 -> drop rgb
            elif drop_mode == 1:
                np_image = np.concatenate((-np.ones_like(np_rgb), np_d), axis=2)
            # mode = 2 -> drop d
            else:
                np_image = np.concatenate((np_rgb, -np.ones_like(np_d)), axis=2)
        ...
        assert np.any(np.isnan(np_image)), f'NaN in input image {sample_id} drop {drop_mode}'
        assert np.any(np.isnan(np_masks)), f'NaN in input mask {sample_id} drop {drop_mode}'
...
if __name__ == "__main__":
    dataset = AutoGeneratedDataset(r'D:\tranduytrung\Mask_RCNN\auto-generated\train', ['3401'])
    x= dataset[0]