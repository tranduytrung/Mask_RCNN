...
from .utils import preprocess_input, obtain_input_shape
...
def MobileNetV2(input_shape=None,
    ...
    # prefix used in the model
    prefix = 'mobilenetv2_'

    if input_tensor is None:
        img_input = layers.Input(shape=input_shape, name=prefix + 'input')
    else:
        if not backend.is_keras_tensor(input_tensor):
            img_input = layers.Input(tensor=input_tensor, shape=input_shape, name=prefix + 'input')
        else:
    ...
    x = layers.ZeroPadding2D(padding=correct_pad(backend, img_input, 3),
                             name=prefix + 'conv1_pad')(img_input)
    stage1 = x = layers.Conv2D(first_block_filters,
                      kernel_size=3,
                      strides=(2, 2),
                      padding='valid',
                      use_bias=False,
    ...
    if include_top:
        ...
        x = layers.Dense(classes, activation='softmax',
                         use_bias=True, name=prefix + 'fc+softmax')(x)
    else:
        if pooling == 'avg':
            x = layers.GlobalAveragePooling2D(name=prefix + 'avgpool')(x)
        elif pooling == 'max':
            x = layers.GlobalMaxPooling2D(name=prefix + 'maxpool')(x)
    ...
    # Load weights.
    if weights == 'imagenet':
        weights_path = get_imagenet_weights_path(include_top, alpha, rows)
...
def get_imagenet_weights_path(include_top=False, alpha=1.0, image_size=224):
        model_name = ('mobilenet_v2_weights_tf_dim_ordering_tf_kernels_' +
                        str(alpha) + '_' + str(image_size) + ('_no_top' if include_top else '') + '.h5')
        weight_url = BASE_WEIGHT_PATH + model_name
...
def _inverted_res_block(inputs, expansion, stride, alpha, filters, block_id, train_bn=None, prefix='mobilenetv2'):
    ...
    x = inputs
    prefix = prefix + 'block_{}_'.format(block_id)
